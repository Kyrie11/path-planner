...
//                                SMOOTHING ALGORITHM
//###################################################
void Smoother::smoothPath(DynamicVoronoi& voronoi, float originX, float originY) {
  ...
  while (iterations < maxIterations) {
    ...
    for (int i = 2; i < pathLength - 2; ++i) {
      ...
      correction = correction - obstacleTerm(xi, originX, originY);
      if (!isOnGrid(xi + correction, originX, originY)) { continue; }
      ...
      if(newPath[i].getPrim() <3) Dxi = -Dxi;
...
void Smoother::tracePath(Node3D* node,float originX, float originY, int i, std::vector<Node3D> path) {
  if (node == nullptr) {
    this->path = path;
    return;
  }
  node->setX(node->getX()+originX);
  node->setY(node->getY()+originY);
  i++;
  path.push_back(*node);
  tracePath(node->getPred(), originX, originY, i, path);
}

//###################################################
//                                      OBSTACLE TERM
//###################################################
Vector2D Smoother::obstacleTerm(Vector2D xi, float originX, float originY) {
  Vector2D gradient;
  ...
  if (x < (width+originX) && x >= originX && y < (height+originY) && y >= originY) {
    Vector2D obsVct(xi.getX() - voronoi.data[(int)(xi.getX()-originX)][(int)(xi.getY()-originY)].obstX,
                    xi.getY() - voronoi.data[(int)(xi.getX()-originX)][(int)(xi.getY()-originY)].obstY);
