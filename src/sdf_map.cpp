...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  grid_pub_ = node_.advertise<nav_msgs::OccupancyGrid>("/map", 1);
...
void SDFMap::clearAndInflateLocalMap() {
  ...
  Eigen::Vector3i min_cut_m = min_cut - Eigen::Vector3i(vec_margin, vec_margin, vec_margin); /*- Eigen::Vector3i(vec_margin, vec_margin, vec_margin);*/
...
void SDFMap::calcSurfaceNormals(pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud, pcl::PointCloud<pcl::Normal>::Ptr normals) {
...
void SDFMap::initGrid(/*nav_msgs::OccupancyGridPtr grid*/) {
  grid->header.seq = 1;
...
void SDFMap::populateMap(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::PointCloud<pcl::Normal>::Ptr cloud_normals, std::vector<int> &countGrid, double xMin, double yMin, int xCells, int yCells) {
  double deviation = 0.78;
  for(size_t i = 0; i < cloud->size(); i++)
  {
    ...
    int xCell, yCell;
    xCell = (int)((x-xMin)/0.05);
    yCell = (int)((y-yMin)/0.05);
    if((yCell * xCells + xCell) > (xCells * yCells))
      std::cout << "x: " << x << ", y: " << y << ", xCell: " << xCell << ", yCell: " << yCell << "\n";

    double normal_value = acos(fabs(z));
    if(normal_value > deviation)
      countGrid[yCell * xCells + xCell]++;
  }
}

void SDFMap::genOccupancyGrid(std::vector<signed char> &ocGrid, std::vector<int> &countGrid, int size) {
  int buf = 5;
...
void SDFMap::updateGrid(/*nav_msgs::OccupancyGridPtr grid,*/ int xCells, int yCells, double originX, double originY, std::vector<signed char> *ocGrid) 
{
  grid->header.seq++;
...
void SDFMap::cloud2Map(/*nav_msgs::OccupancyGridPtr grid, */pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, double xMin, double yMin, double xMax, double yMax) {
  pcl::PointCloud<pcl::Normal>::Ptr cloud_normals(new pcl::PointCloud<pcl::Normal>);
  calcSurfaceNormals(cloud, cloud_normals);
  int xCells = ((int)((xMax - xMin) / mp_.resolution_)) + 1;
  int yCells = ((int)((yMax - yMin) / mp_.resolution_)) + 1;
  std::vector<int> countGrid(yCells * xCells);
  populateMap(cloud, cloud_normals, countGrid, xMin, yMin, xCells, yCells);
  ...
  //   double x = cloud->points[i].x;
  //   double y = cloud->points[i].y;
  //   double z = normals->points[i].normal_z;
...
void SDFMap::calcSize(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, double *xMax, double *yMax, double *xMin, double *yMin) {
...
void SDFMap::publishMap() {
  ...
  double xMin = SHRT_MAX, yMin = SHRT_MAX, xMax = SHRT_MIN, yMax = SHRT_MIN;
  ...
  for (int x = min_cut(0); x <= max_cut(0); ++x)
    for (int y = min_cut(1); y <= max_cut(1); ++y)
      for (int z = min_cut(2); z <= max_cut(2); ++z) {
        ...
        if (md_.occupancy_buffer_inflate_[toAddress(x, y, z)] == 0) pt.z=0;
        xMin = pos(0) < xMin ? pos(0) : xMin;
        yMin = pos(1) < yMin ? pos(1) : yMin;
        xMax = pos(0) > xMax ? pos(0) : xMax;
        yMax = pos(1) > yMax ? pos(1) : yMax;
        cloud.push_back(pt);
      }
  //SDFMap::cloud2Map(cloud);
  // std::cout<<"In fastplanner xMin is:"<<xMin<<",yMin is :"<<yMin<<std::endl;
  ...
  // cloud2Map(/*grid,*/ cloudPtr, xMin, yMin, xMax, yMax);
...
void SDFMap::publishMapInflate(bool all_info) {
  ...
  double xMin = SHRT_MAX, yMin = SHRT_MAX;
  ...
  // pcl::PointCloud<pcl::PointXYZ>::Ptr cloudPtr;
  // cloudPtr = cloud.makeShared();
  // cloud2Map(grid, cloudPtr, xMin, yMin);
  // ROS_INFO("pub map");
}
