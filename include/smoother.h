...
class Smoother {
 public:
  ...
  /*!
     ...
     voronoiCost
  */
  void smoothPath(DynamicVoronoi& voronoi, float originX, float originY);
  ...
  */
  void tracePath(Node3D* node, float originX, float originY, int i = 0, std::vector<Node3D> path = std::vector<Node3D>());
  ...
  Vector2D obstacleTerm(Vector2D xi, float originX, float originY);
  ...
  bool isOnGrid(Vector2D vec, float originX, float originY) {
    if (vec.getX() >= originX && vec.getX() < (width + originX) &&
        vec.getY() >= originY && vec.getY() < (height + originY)) {
      return true;
    } 
    return false;
  }
