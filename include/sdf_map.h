...
public:
  ...
  void calcSurfaceNormals(pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud, pcl::PointCloud<pcl::Normal>::Ptr normals);
  void populateMap(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::PointCloud<pcl::Normal>::Ptr cloud_normals, std::vector<int> &countGrid, double xMin, double yMin, int xCells, int yCells);
  void genOccupancyGrid(std::vector<signed char> &ocGrid, std::vector<int> &countGrid, int size);
  void updateGrid(/*nav_msgs::OccupancyGridPtr grid,*/ int xCells, int yCells, double originX, double originY, std::vector<signed char> *ocGrid);
  void cloud2Map(/*nav_msgs::OccupancyGridPtr grid,*/ pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, double xMin, double yMin, double xMax, double yMax);
  void calcSize(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, double *xMax, double *yMax, double *xMin, double *yMin);
  void publishMap();
...
private:
  ...
  ros::Publisher map_pub_, esdf_pub_, map_inf_pub_, update_range_pub_, grid_pub_;
...
inline void SDFMap::indexToPos(const Eigen::Vector3i& id, Eigen::Vector3d& pos) {
  for (int i = 0; i < 3; ++i) pos(i) = (id(i)) * mp_.resolution_ + mp_.map_origin_(i);
}
